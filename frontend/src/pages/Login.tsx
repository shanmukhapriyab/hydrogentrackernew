...
const DEMO_ACCOUNTS: { role: UserRole; label: string; email: string; description: string }[] = [
  { role: 'admin', label: 'Administrator', email: 'a.mitchell@h2track.com', description: 'Full platform access' },
  { role: 'producer', label: 'Producer', email: 'c.reyes@h2track.com', description: 'Production & storage management' },
  { role: 'logistics', label: 'Logistics', email: 'p.sharma@h2track.com', description: 'Shipment & delivery management' },
  { role: 'customer', label: 'Customer', email: 't.berg@h2track.com', description: 'Order & delivery tracking' },
];

export default function Login({ onLogin }: LoginProps) {
  const [email, setEmail] = useState('a.mitchell@h2track.com');
  ...
  const handleLogin = () => {
    setLoading(true);
    setTimeout(() => { onLogin(selectedDemo); }, 800);
  };
      ...
      <div className="hidden lg:flex flex-col w-[480px] bg-blue-700 text-white p-12 relative overflow-hidden flex-shrink-0">
        ...
        <div className="relative z-10 flex-1 flex flex-col">
          <div className="flex items-center gap-3 mb-16">
            ...
            <div>
              <div className="font-bold text-lg">HydrogenTrack</div>
              <div className="text-blue-200 text-xs">Supply Chain Platform</div>
            </div>
          </div>

          <div className="flex-1">
            <h2 className="text-3xl font-bold leading-tight mb-4">
              Hydrogen Supply Chain,<br />Fully Visible.
            </h2>
            ...
            <div className="space-y-4">
              {[
                { label: 'Production Monitoring', value: '6 active plants' },
                { label: 'Active Shipments', value: '3 in transit today' },
                { label: 'Platform Uptime', value: '99.97% SLA' },
              ].map(item => (
                <div key={item.label} className="flex items-center gap-3">
                  <div className="w-1.5 h-1.5 bg-blue-300 rounded-full" />
                  <span className="text-blue-100 text-sm">{item.label}</span>
                  <span className="ml-auto text-white font-medium text-sm">{item.value}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="w-full max-w-sm">
          <div className="mb-8">
            <div className="flex items-center gap-2 mb-6 lg:hidden">
              ...
              <span className="font-bold text-slate-900">HydrogenTrack</span>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-4">
            ...
            <div className="flex items-center gap-2">
              <input type="checkbox" id="remember" className="rounded border-slate-300 text-blue-600" defaultChecked />
          ...
          {/* Demo accounts */}
          <div className="mt-5">
            <p className="text-xs font-medium text-slate-500 mb-3 text-center">Quick demo access</p>
            <div className="grid grid-cols-2 gap-2">
              {DEMO_ACCOUNTS.map(acc => (
                ...
                >
                  ...
                  <div className="text-slate-400 mt-0.5 text-[10px]">{acc.description}</div>
                </button>
              ))}
            </div>
          </div>
